...
- Lesson catalogue size and translation volume ratio (10.4) — **you do not refresh these by hand, and
  ...
  fails and names the replacement string. To see the numbers, or to update the document after a
  content change:
  ```bash
  npm run readiness            # print them
  npm run readiness -- --write # put them into this file
  ```
  The script imports `content/lessons.js` (id/track/icon/color/minutes/title/subtitle) and
