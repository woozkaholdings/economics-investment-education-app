...
- Lesson catalogue size — **as of 2026-08-17 (item 45), lesson body text lives in ten files,
  `content/lessonContent.<track>.<lang>.js`. The commands below deliberately read
  `content/lessonContent.js`, which survives as the node-only merged view of all ten** — it is the one
  source that can see every language at once, which is exactly what a volume measurement needs.
  **This paragraph has now been stale twice, both times naming a path that no longer existed**: it
  described the single pre-2026-08-14 file for three days after item 25 split it per track, and the
  two per-track files for the hours between item 45's per-language split and this correction. Both
  times the commands below would have thrown `ERR_MODULE_NOT_FOUND` rather than returning a stale
  number — **run them, do not trust this text** (that is the whole argument for backlog item 39's
  recommendation: this snippet keeps breaking because it is a copy of code, and nothing executes it).
  `content/lessons.js` still holds only id/track/icon/color/minutes/title/subtitle. Read both:
  ```bash
  ...
  ```
- Translation volume ratio (10.4) — same merged-view source:
  ```bash
  BIN_DIR="$(scripts/bootstrap-node.sh)"; export PATH="$BIN_DIR:$PATH"
  node -e '
  Promise.all([import("./src/content/lessons.js"), import("./src/content/lessonContent.js")]).then(([{lessons}, {lessonContent}]) => {
