...
**Last refreshed: 2026-08-23 (scheduled dev-agent run, backlog W-5.6).** This pass changed **one row's
narrative and nothing else**: §10.4's closing clause, which framed the five-language surface as "real
ongoing maintenance debt … just a cost to keep tracking". That framing is what backlog item 93
disproved — the shortfall is *absent content*, it is *concentrated* on the optional track, and it is
therefore schedulable rather than perpetual. §10.4 now carries the per-language reference ratios and
the abridged-pair count that make its aggregate figures readable. The generated figures in this file
were **not** hand-edited (they are owned by `scripts/refresh-readiness.mjs`; see "How to refresh this
file") and were re-verified as current against live content this date.
