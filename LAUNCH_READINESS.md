...
| 10.4 | Five languages = maintenance debt | 🟡 Open, tracked | es/ko/zh/ja labelled "(Beta)" in the language picker. Volume ratio **re-measured 2026-08-06** after lesson 22 was added with translations on every field: es 21,909 chars (0.41x of English), ko 11,415 (0.22x), zh 7,270 (0.14x), ja 9,432 (0.18x) — all four essentially flat versus lesson 21's measurement, since this run (like the last four) translated the new lesson in step rather than leaving it English-only. See item 20 in `AGENT_LOG.md`. |
...
|---|---|---|---|
| Lesson catalogue size | ≥40 lessons / ~2 hours | **22 lessons / 53,020 English chars / ~44 min** | ❌ Not met — roughly 60% of the char/time target, 55% of the lesson-count target |
