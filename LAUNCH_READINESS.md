...
**Last refreshed: 2026-08-06 (lesson 25 added, item 17).**
...
| 10.4 | Five languages = maintenance debt | 🟡 Open, tracked | es/ko/zh/ja labelled "(Beta)" in the language picker. Volume ratio **re-measured 2026-08-06** after lesson 25 was added with translations on every field: es 31,497 chars (0.50x of English), ko 15,696 (0.25x), zh 10,018 (0.16x), ja 13,195 (0.21x) — all four essentially flat-to-slightly-up versus lesson 24's measurement, since this run (like the last seven) translated the new lesson in step rather than leaving it English-only. See item 20 in `AGENT_LOG.md`. |
...
|---|---|---|---|
| Lesson catalogue size | ≥40 lessons / ~2 hours | **25 lessons / 62,938 English chars / ~57 min** | ❌ Not met — roughly 77% of the char/time target, 63% of the lesson-count target |
...
forward from the last run-log claim. Reading-time estimate uses ~5.5 chars/word at 200 wpm. Cross-checked
against the app's own per-lesson `estimateMinutes` (summed): 52 min — the two methods still drift apart
by about the same margin as at 24 lessons (57 vs. 52), consistent with the drift noted since this
cross-check was first added; still close enough to trust both.
