...
**Last refreshed: 2026-09-04 (scheduled dev-agent run, self-picked from a live sweep of the built app).**
This pass fixed **one inverted evidence cell and three stale figures**, and the inverted cell is the finding:
§4.3's *structural precondition* row described a new install opening into the **money** track, seventeen days
after the 2026-08-18 reversal made **economy** the opening path. The full correction, its control, and why the
row's verdict survived while its reasoning did not, are in the note under that table. Also refreshed in §10.4,
whose *narrative* is hand-written and therefore unguarded: **12 of 44 lessons** (was "of 40" — the corpus grew
by the four money lessons of 2026-08-25) and the p90 full-translation references **ko 0.55 → 0.58, zh 0.35 →
0.36, ja 0.50 → 0.51**, re-run this date with `npm run translation-completeness`. **The abridged set itself did
not move** — still 48 pairs, es/ko/zh/ja 12 each, still entirely on the optional `essentials` track (lessons
1-11 and 14) — so §10.4's conclusion is unchanged and only its arithmetic was stale.
Prior refresh 2026-08-24 (scheduled dev-agent run, backlog item 93 — the economy phase closes).
This pass changed **one row's figures and one clause of its narrative**: §10.4 now reads **48 abridged
pairs — es 12, ko 12, zh 12, ja 12 across 12 of 44 lessons**, and the clause that named `ja` 39-40 as
...
| 10.4 | Five languages = maintenance debt | 🟡 Open, tracked (translation coverage), but the owner decision this row used to flag is resolved | es/ko/zh/ja labeled "(Beta)" in the language picker. Volume ratio **generated, not typed** — `npm run readiness` computes the sentence below from the live corpus and `npm test` fails if this row disagrees with it, so it carries **no measurement date, and none should be added**. (Until 2026-09-04 this clause read "re-measured 2026-08-16 over all 40 lessons" beside figures that had been regenerated continuously through four added lessons and a whole translation phase — the generated half was right the entire time and the sentence introducing it was wrong.) Sourced from `content/lessonContent.js`, the node-only merged view of the per-track, per-language `content/lessonContent.<track>.<lang>.js` files (split per track 2026-08-14 by item 25, per language 2026-08-17 by item 45 — see `DECISIONS.md`; **the file count is deliberately not restated here** — it said "ten" from the two-track era until 2026-09-04, when `ls` showed 15): **es 183,449 chars (1.116x of English's 164,401), ko 88,122 (0.536x), zh 55,200 (0.336x), ja 77,262 (0.470x)**, zero missing fields in any language (re-run 2026-08-16 with this file's own documented command, against committed content). Every non-English figure fell by **exactly 19 characters** that day while English stayed identical to the character — that is item 36's stale-cross-reference fixes shortening two-digit lesson references to one digit, not a measurement change or a translation edit. **P-3 (above) and P-4 are both done**: the blindspot check now scans all five languages, and the owner resolved the machine-translation question 2026-08-11 — option (a), accept the current unreviewed state under "(Beta)" labeling, tracked going forward via `scripts/translation-review.mjs`'s ledger. Live coverage: **es 100% (0% human, 0 stale), ko 100% (0% human, 0 stale), zh 100% (0% human, 0 stale), ja 100% (0% human, 0 stale)**. **Back to 100% as of 2026-08-16 (owner-requested re-review).** It had dropped to 93% earlier that day, and the drop was the ledger working rather than a regression: English bodies for lessons 5, 27 and 28 were edited after their reviews (cross-reference fixes, backlog items 33 and 36), so the ledger flagged those 12 pairs stale instead of continuing to count them as covered — the first time drift detection had fired since it was built for P-4. The re-review confirmed the English change in all three lessons was **only** lesson-reference numbers, no prose, and that each translated reference now names the lesson its own surrounding text describes; see this date's run log for the method. `check-data.mjs` §11 now fails the build if this sentence disagrees with the live ledger, so it cannot silently go stale again (this row's figure was three weeks' worth of unnoticed drift when backlog item 37 caught it); run `npm run review-status` for per-lesson detail. See "former item 20" in `AGENT_LOG.md`'s Completed-and-pruned section and `DECISIONS.md` for the full writeup. **Remaining open part of this row — re-framed 2026-08-23 (backlog W-5.6), because the aggregate ratios above cannot be read without a per-language reference and were being read as compactness.** Against nothing, the `zh` ratio in the generated sentence above reads as Chinese simply being a denser language. (That ratio was restated here as `0.294x` until 2026-09-04, by which time the generated figure it was quoting had moved.) Against **this corpus's own fully-translated lessons in Chinese** it means roughly a sixth of the content is absent. `scripts/translation-completeness.mjs` supplies that reference: each language's **p90 ratio across all 44 lessons — what a full translation actually looks like here** — is **es 1.16, ko 0.58, zh 0.36, ja 0.51** (re-read from the instrument 2026-09-20; the `es` figure had been typed here as 1.18 and the live p90 has since moved — run the command rather than quoting this), and a lesson counts as *abridged* below 0.7x its own language's reference. Measured 2026-08-24: **48 abridged pairs — es 12, ko 12, zh 12, ja 12 — across 12 of 44 lessons** (`npm run translation-completeness`). ⚠️ **Re-measured 2026-09-05: 47 — es 11, ko 12, zh 12, ja 12, still across the same 12 lessons.** The pair that left is **lesson 14's `es`, whose Spanish was not edited** — it reads `0.83` both times. ⚠️ **CORRECTED 2026-09-22: that `0.83` was a threshold artifact, and reading it as "Spanish is fine here" misled this project for seventeen days.** Spanish lesson 14 was abridged exactly as the other three were, missing the same clauses in the same three paragraphs — but `es` expands to ~116% of English in this corpus, so dropping about a quarter of the content still scored `0.83` against a `0.812` cut, clearing it by 2.2%. Backlog item 94 inherited that reading and budgeted the closing run as three languages; measured per paragraph it was four. **A lesson-level ratio is structurally blind to within-paragraph deletion in whichever language has the largest expansion factor** — the per-paragraph instrument that does see it, and its control, are in the 2026-09-22 run-log entry. The threshold is `0.7 x` a **corpus-wide p90**, so editing *any* lesson's English or Spanish moves the bar every other pair is judged against; a two-sentence fix in lesson 37 reclassified lesson 14. **Read a change in this count as a threshold move until a per-lesson diff says otherwise** — it is not by itself evidence that any translation got longer. ⚠️ **Re-measured 2026-09-22, after twelve lessons closed across 2026-09-20/21/22: 0 — es 0, ko 0, zh 0, ja 0, across 0 of 44 lessons.** Every lesson in the corpus now clears the completeness threshold in all five languages. Unlike the 48→47 move above, **this one is a per-lesson diff and not a threshold move**: lesson 1 (es 0.61→1.05, ko 0.30→0.53, zh 0.20→0.33, ja 0.26→0.46) then lesson 6, the worst-abridged pair set in the corpus (es 0.41→1.14, ko 0.20→0.54, zh 0.12→0.34, ja 0.17→0.45), then lesson 10 (es 0.57→1.07, ko 0.27→0.51, zh 0.16→0.31, ja 0.24→0.44, whose English opener was rewritten concrete-first in the same pass), then lesson 9 (es 0.54→1.06, ko 0.28→0.50, zh 0.17→0.30, ja 0.25→0.44), then lesson 3 (es 0.61→1.08, ko 0.29→0.53, zh 0.19→0.34, ja 0.25→0.44), then lesson 2 (es 0.56→1.10, ko 0.29→0.51, zh 0.20→0.33, ja 0.26→0.44), then lesson 8 (es 0.68→1.08, ko 0.33→0.50, zh 0.21→0.31, ja 0.28→0.43), then lesson 5 (es 0.80→1.09, ko 0.36→0.50, zh 0.23→0.32, ja 0.31→0.43), then lesson 7 (es 0.79→1.08, ko 0.37→0.51, zh 0.24→0.34, ja 0.32→0.45), then lesson 4 (es 0.75→1.04, ko 0.37→0.49, zh 0.25→0.32, ja 0.34→0.44), then lesson 11 (es 0.81→1.13, ko 0.38→0.51, zh 0.25→0.33, ja 0.34→0.45), and finally lesson 14 (es 0.83→1.10, ko 0.37→0.49, zh 0.24→0.32, ja 0.33→0.43), were each fully translated in all four languages, landing on top of in-track references 12/13/15 rather than on the threshold — so forty-seven pairs left the list because the work was done — forty-four of them plus lesson 14's three, `es` 14 having never been on the list to leave (see the correction below). **The shortfall is concentrated, not spread, and that is the whole reason it is schedulable:** **no abridged pairs remain on any track** — the last, lesson 14, closed 2026-09-22 — and both main-path tracks reached **fully translated in all four languages (0 abridged pairs)** before it — `money` (16-28) since 2026-08-22 and `economy` (29-40) as of 2026-08-24, when `ja` 39-40 closed the last gap. The abridged set is now **identical in all four languages**, which is what makes the remainder one schedulable block rather than four. So the honest statement of this row is not "ongoing maintenance debt" but: *all three tracks are now fully translated in all five languages by this instrument's threshold — and every non-English word of it is still machine translation no fluent speaker has read, which is O-3's open question and not this row's.* ⚠️ **That clause read "about four-fifths absent" until 2026-09-20, and by then it was the mirror image of the truth** — twelve of the fifteen `essentials` lessons have been fully translated since it was written, so measured against each language's own p90 reference the track now carries **91.1% of a full translation's volume (es 94.4%, ko 88.3%, zh 90.1%, ja 87.3%)** — summed translated characters (headings, bodies, takeaway and thinkAbout) over lessons 1-15 divided by (summed English characters × that language's p90), **with the four languages summed before dividing rather than their four fractions averaged**, which is the method and now says which aggregation it means. ✅ **RESOLVED 2026-09-22, after four consecutive passes recorded this as unreproducible drift and wrote a warning instead of a diagnosis: the level was never drifting — the passes were aggregating the four languages differently.** Re-run on the pre-edit state, the stated method reproduces the predecessor's **89.8% exactly** when the four languages are summed before dividing, and reads **88.8%** when the four per-language fractions are averaged instead — a 1.0pp gap of pure arithmetic convention. **What identifies it as the aggregation step and not the measurement is that all four per-language figures reproduced exactly** (es 93.0%, ko 87.1%, zh 88.8%, ja 86.1% — the published values to the decimal). The recorded 0.2pp / 0.2pp / 1.1pp / 0.9pp "failures to reproduce" were the same ambiguity at smaller corpus sizes. The method sentence above now names the aggregation, so the level is a series again. Still re-measure with `npm run translation-completeness` rather than quoting it. **Note on the two character figures in this row:** the generated `es … chars` sentence above comes from `scripts/refresh-readiness.mjs`, which counts section **bodies** plus takeaway/thinkAbout; `translation-completeness.mjs` also counts section **headings**, so it reports a larger English corpus. **The gap is exactly the headings** — re-verified 2026-09-04 by summing them independently and reproducing *both* instruments' English totals to the character — and the per-language ratios agree to within half a point either way, so the two instruments describe the same surface, not two different ones. **No figure from either instrument is restated in this note, on purpose:** it previously quoted four of them (`es 134,697`, `140,700`, `137,249`, `3,451`) and **all four had gone stale** while the generated sentence two inches above stayed correct — a note explaining why the numbers can be trusted, written in numbers that could not. Run `npm run readiness` and `npm run translation-completeness` for the live values. |
...
|---|---|---|---|
| Lesson catalog size | ≥40 lessons / ~2 hours | **44 lessons / 164,401 English chars / 174 min** — split across **money (17)** + **economy (12)** + **essentials (15)** tracks, see `LAUNCH_PLAN.md` §2.5 | ✅ Both §4.3 content clauses **met** — lesson-count (40/40) since 2026-08-09, minutes (**144**/120) since 2026-08-15, when the clause first cleared at 120. The jump from 120 to 144 on 2026-08-17 is not new content: backlog item 56 found the `minutes` model omitted section headings and the whole end-of-lesson check (~20% of the words on screen) and recalibrated all 40 lessons — see `DECISIONS.md`, "How a lesson's `minutes` estimate is computed". Completion-rate clause (item 18) remains unmeasured — see row below. |
| Installer finishes lesson 1 — *structural precondition* | Lesson 1 should be worth finishing | **Re-measured live on the built app 2026-09-04, and this cell's previous text was inverted — see the note under this table.** A new install opens into the **economy** track's first lesson ("Transactions: The Building Block", `How the Economy Works`), which is what the 2026-08-18 reversal in `DECISIONS.md` intends. The structural obstacle is gone because the tracks are **independent**, not because of which one opens first: from cleared storage, `Thinking About Money`'s lesson 1 is **unlocked with zero lessons completed** (16 of its 17 rows locked, the first not), so no money lesson sits behind the macro chain. | ⚠️ Still unmeasurable (no analytics pipeline), but the structural obstacle is gone |
...
> ⛔ **The "structural precondition" row above said the opposite of what the app does, from 2026-08-18 to
> 2026-09-04 — seventeen days — and it is exactly the drift §10.7 exists to catch.** It read *"A new install
> opens into the money track's first lesson (fixed 2026-08-07), not the macro-theory chain"*. That was true when
> written and was reversed by the owner-directed product decision of **2026-08-18** (`DECISIONS.md`), which made
> `economy` the main path; `LAUNCH_PLAN.md` §0 and `DECISIONS.md` were both updated at the time and **this file
> was not**. Measured 2026-09-04 by driving the built app from cleared `localStorage`, with a control proving the
> reading was live rather than a constant (seeding lesson 29 as complete moved the card from "START HERE /
> Transactions" to "NEXT UP / Credit" and the counter from 0/44 to 1/44).
> **The verdict did not change and the reasoning did**, which is why this is worth writing down rather than
> silently overwriting: the row's ⚠️ rested on *which track opens first*, and the property that actually clears
> the obstacle is that **money is not gated behind economy at all**. Had the two ever come apart — a reversal that
> also re-gated the tracks — this row would have scored the gate on a premise it no longer had.
> **Transferable, and it is this file's own rule turned on itself:** the "How to refresh this file" section below
> guards the two *generated* figures with `npm test` and leaves every *narrative* evidence cell unguarded. A
> generated figure cannot go stale here; a hand-written mechanism can, and did, in the cell scoring a Phase-0 gate.
...
55), and since the same date `CLAIMS.md`'s A6 status cell. It was extended because the two
documents had *disagreed*: this file said both §4.3 content clauses were met while the plan still
said "the gate is not close: 12 minutes is not 2 hours." The generated figures share one
measurement, so that particular contradiction cannot recur.

The inventory is itself generated, for the reason this paragraph is the evidence for — until
2026-09-04 it read "eight more" and "Ten generated figures", when the true counts were nine and
twelve, and it did not mention `CLAIMS.md` at all. A paragraph asserting that the figures beside
it are not hand-typed was three hand-typed claims, two of them wrong:

> **Generated-figure inventory: 13 figures — LAUNCH_READINESS.md 3, LAUNCH_PLAN.md 9, CLAIMS.md 1.**
