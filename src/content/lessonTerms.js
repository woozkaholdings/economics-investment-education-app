...
//
// COVERAGE, re-measured 2026-08-17 (item 57): 44 chips across 22 lessons — 13
// money, 9 economy — against 77 glossary-term uses in all, the other 33 of
// which are deliberate and listed at the bottom of this file. The previous
// figure here ("21 links across 10 lessons — 9
// economy, 1 money") was written earlier the same day and was already stale
// when committed: it described the map as it stood *before* the item-35
// expansion listed directly below it, which is why the "lopsidedness" it went
// on to explain is no longer there to explain. Both numbers are generated by
// `scripts/check-data.mjs` §17b, which prints them every `npm test` run —
// re-read them from there rather than trusting this sentence.
//
...
export const lessonTerms = {
  ...
  // see the 29 keys in glossary.js, so a jargon word with no glossary entry is
  // invisible to it. That residual is a real limit, not a closed one.
