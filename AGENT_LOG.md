...
### 2026-09-19 (scheduled dev-agent; W-6.2 rule 1 does not arise: the previous run was the owner-directed archiving pass, and all three of its closing notes route to the owner — O-6's automation question, "the next firing is not imminent", and the floor — so none of them is a residual this run could take. This was a free pick, from a scan of every English sentence in the `money` track carrying a frequency, superlative or history word, for 5-word runs appearing **nowhere** in `AGENT_LOG.md`, `AGENT_LOG.archive.md`, `CLAIMS.md`, `DECISIONS.md` or `LAUNCH_PLAN.md`. Controls fired both ways: `the point of maximum pessimism` returns 3, a nonsense phrase returns 0) — lesson 23 opened by telling learners, as a fact about people, that **"most people take the $50 today"** over **$65 in one month**. That is a **30% premium for one month's wait**, and the measured record does not support turning it down. **Two arithmetic claims checked on the way were exactly right and were left alone**: lesson 16's `$300/month × 36 = $10,800`, and lesson 19's `$2,000 × 1.06^10 = $3,581.70` against its "roughly $3,580"

**Step 3.5 — the premise re-measured before anything was edited, with controls.**
- **Instrument:** Frederick, Loewenstein & O'Donoghue (2002), *Time Discounting and Time Preference: A Critical Review*, JEL XL — the canonical survey. The fetcher returned the PDF as binary and **refused to answer rather than inventing one**, which is itself the negative control on the fetch; the text was then extracted locally by inflating the FlateDecode streams (`scratchpad/pdftext.cjs`, 190,239 chars). **Extraction controls fired in both directions:** `Time Discounting` 14, `Thaler` 16, `magnitude effect` 6, `discounted utility` 3, and a nonsense string **0**.
- **What it says, verbatim.** §4.1: *"Richard Thaler (1981) asked subjects to specify the amount of money they would require in [one month/one year/ten years] to make them indifferent to receiving $15 now. The median responses [$20/$50/$100] imply an average (annual) discount rate of 345 percent over a one-month horizon"*. So at a $15 stake the median person is **indifferent at a 33.3% one-month premium** — and the app offers **30%**, i.e. just *under* the median indifference point even before stake size is considered.
- **The magnitude effect is what decides it, and it is measured, not asserted.** §4 lists the anomalies as *"gains are discounted more than losses, **small amounts more than large amounts**"*; §4.2.2 is headed *"The Magnitude Effect (small outcomes are discounted more than large ones)"* and gives Thaler's own one-year indifference points: **$15 ↔ $60, $250 ↔ $350, $3,000 ↔ $4,000** — required premiums of **300%, 40% and 33%**, implied rates **139%, 34%, 29%**. A $50 stake therefore needs **less** than the 33.3% a $15 stake needs, so a 30% premium at $50 sits **above** the median person's indifference point and most would **wait for the $65** — the opposite of what the lesson said.
- ⚠️ **The honest limit, stated rather than rounded in my favor.** The two facts above are direct quotations. The conclusion about the **$50/one-month cell specifically is an interpolation between them**, not a measurement of that cell — no study in the review reports it. What is certain is the weaker, sufficient claim: **nothing in the record supports "most people", and the two nearest measured points bracket the app's number on the wrong side.** The fix is written to need only that weaker claim.
- **A third measured fact shaped the fix.** The review's own illustration of exactly the reversal lesson 23 teaches (§4.1) is *"someone may prefer $110 in 31 days over $100 in 30 days, but also prefer $100 now over $110 tomorrow"* — a **10% premium over one day**, an order of magnitude gentler than the app's 30%-over-a-month. The lesson had invented a steeper example than the literature uses, and then reported it as what people do.

#### The disposition, and why it is NOT "change the numbers"
The obvious fix — re-base the example on the published $100/$110-over-a-day shape — **was considered and rejected as out of scope for one run**, and the reason is a constraint the figure imposes rather than a preference. `moneyVisuals.js`'s `flipRewards` is `{50, month 12}` / `{65, month 13}`, and `check-data.mjs` §50 block (a) **fails the build** if it is not 50/65 (*"Change the lesson and the figure together or not at all"*), while block (i) **fails** unless lesson 23's body states **50, 65 and 15 in every one of the five languages**. Changing the amounts means the five bodies, `flipRewards`, `flipDiscountK`, the title, caption, description, axis, series and zone labels in five languages, the §50 constants and quiz `q025` — in one commit, on the repo's most heavily instrumented figure. **So this run fixed the claim about people and left the arithmetic alone**, which is the part the measurement actually condemns: the figure plots what two options *feel* worth under one discount function, which is a model and was never a population claim.

#### What shipped (8 files, no script and no check added — W-6.3: **0 lines** to `scripts/`)
- **Lesson 23 §0, all five languages.** "most people take the $50 today" → **"some people take the $50 today"**, and "most people switch" → **"the answer tends to flip"**. A new second paragraph says the flip is the well-established part, that the figures are **stretched to make it easy to see**, and gives the measured anchor: people needed **$20 in a month to give up $15 today — a third more for a single month's wait** — while the same study found **only about a third more to wait a whole year when the amount was $3,000**, so **small sums get discounted far more steeply than large ones**. The source is attributed as *"a much-cited 1981 experiment"* rather than by name: **the app has no learner-visible researcher name anywhere** (the one `Shiller` hit in `src/` is a data-series name in a code comment), and introducing one is a house-style change this run did not set out to make. Thaler (1981) is named here so a future run does not have to re-derive it.
- **`flipZoneLabels`, all five languages** — the same false claim on the figure surface: *"Here, most people wait for the $65"* / *"Here, most people take the $50"* → **"Here, the $65 feels worth more"** / **"Here, the $50 feels worth more"**. This matches the figure's own title (*"What each option **feels** worth"*) and its existing footer, *"Example figures, for teaching — not a projection of your own results"*, which the body now agrees with instead of contradicting.
- **`moneyVisuals.js` design comment.** The `k > 0.3` bound is *derived from the sentence this run changed* (*"Preferring $50-now over $65-in-a-month requires … k > 0.3"*), so leaving it unannotated would have left a true comment resting on a claim that had quietly moved. It now records the date, the measurement and that the bound still holds — **it is now that one person's preference rather than most people's.**
- `LAUNCH_PLAN.md` / `LAUNCH_READINESS.md`: generated figures only, via `npm run readiness -- --write` — **4 lines across 2 files**, the English char count, the word count and the per-language volume sentence. No ratio cascade.
- Ledger L23 es/ko/zh/ja re-marked `ai` (the English source hash moved, so `npm test` correctly flagged all four stale).

#### Verification
| Check | Result |
|---|---|
| `npm test` | **exit 0**, 0 FAIL, **3 WARN** — the same three as the pre-edit baseline, byte for byte in kind (coverage 100%/0% human, completeness 47, quiz length-cue 52.2%). **No new warning** |
| §50 block (j) — **proved live, not assumed** | §50 prints **no** summary line, so its silence proves nothing. **Plant: zh zone labels swapped → 4 FAILs**, naming `flipZoneLabels.zh[0]`/`[1]` exactly |
| §50 block (i) — proved live | **Plant: every `15ドル` removed from ja lesson 23 → FAIL** *"lesson 23's own body never states $15 (the difference the lesson turns on) in \"ja\""* |
| Files pristine after both plants | `cmp` against scratchpad post-edit copies: **all 6 identical** |
| Reading-time model | `minutes` is a **hard FAIL** if it disagrees (`check-data.mjs` §2), and the suite passed — so L23 still computes to 4 min after +583 English chars. Positively validated, not silently skipped |
| `npm run check-blindspot` | **exit 0**. §10.1 clean across all 33 advice patterns in five languages; **§2.3 clean across the 26 teaching-copy modules including the ones I edited** — the "1981" I added is historical, not live-looking |
| Build | `npm run build` fails on this host (iCloud-synced `node_modules` holds one CPU's rollup binary); **`scripts/build-out-of-tree.sh` → ✓ built in 590ms**, mirrored to `dist/` |
| Live render | Served `dist/` statically (control: a missing path → **404**) and read the real DOM at `#/lesson/23` in **en and zh**: both new paragraphs present, both zone labels reworded, no mojibake in the CJK. ⚠️ **Screenshots came back blank** — the Browser pane pauses rAF, so the animated lesson body never paints; the text extraction is the instrument that works here and the visual check is **not** claimed |

#### Step 5: adversarial self-check
- **Blindspot register.** §10.1 — the new prose is descriptive ("small sums get discounted far more steeply than large ones"), contains no recommendation, and `check-blindspot` passes in all five languages. §10.2 — **no name-brand dependency added; I considered naming Thaler in the lesson and deliberately did not.** §10.3 — untouched. Stale-data rule — §2.3 passes over the exact modules edited.
- **DECISIONS.md.** No conflict: `.js` content modules edited in place, `localStorage` and Vite untouched, and the four new translations are marked `ai`, which is what the 2026-08-11 option-(a) decision provides for.
- **Already-done backlog item.** `Thaler`, `magnitude effect` and `most people take the $50` each return **0** in `AGENT_LOG.md`, the archive, `CLAIMS.md`, `DECISIONS.md` and `LAUNCH_PLAN.md` — this literature has never been consulted here and this sentence was never measured. The archive's 36 `lesson 23` hits are the **figure's geometry** (items 27, 137, 150, 151), and this run changes none of it: `flipRewards`, `flipDiscountK`, `flipMonths`, `flipValue`, `flipCrossing` and `flipYNorm` are byte-identical.
- **My own verification claim.** A reviewer re-running my commands gets the same result: the review is at a public URL, the extractor is four lines of `zlib`, and both §50 plants are one string swap each. **The one thing they must not take from me is the interpolated $50/one-month conclusion** — it is flagged above as an interpolation, and the edit was written so it does not depend on it.
- **Am I padding?** The diff is 8 files and the prose change is three sentences plus one paragraph. It is written up at this length because the measurement is the expensive half and the *rejected* larger fix needs its reason on the record, so the next run does not re-derive the §50 constraint.

#### Seen, deliberately NOT fixed and NOT numbered (W-6.2 rule 2)
- **The example's numbers are still steeper than the literature's**, and re-basing them on FLO's published *"$100 now vs $110 tomorrow / $100 in 30 days vs $110 in 31 days"* is the real fix. It is blocked behind §50 block (a)'s "change the lesson and the figure together or not at all" — five bodies, `flipRewards`, `flipDiscountK`, seven figure-text keys × five languages, the §50 constants and quiz `q025`. **A genuine one-run item for a run that wants it; all the measurement it needs is in this entry.**
- **Quiz `q025`/`q012`-style surfaces were checked and are clean here:** the lesson-23 quiz question narrates **Priya**, a named hypothetical, and never claims what most people do. No change needed.
- **`sectors.js` is the least recently touched content module** (2026-08-27) and read clean on inspection — eleven definitional sector blurbs with nothing measurable to drift. Noted so the next module-age ranking does not re-open it as a lead.

**Owner-facing, one line:** one lesson was telling learners that most people would turn down a 30%-a-month return, as if it were a measured fact about people. It is not: in the standard reference experiment the typical person is roughly indifferent at that rate for a *smaller* sum, and larger sums make people *more* willing to wait — so the real answer is probably the opposite. The lesson now says "some people", tells the reader the dollar figures are stretched for illustration, and gives the actual measured numbers; the chart's two labels stopped saying "most people" too. **The chart's math is unchanged and correct — it was always a model of how the options *feel*, not a claim about a population.** **Committed, not pushed** (O-5).

**Schedule:** the cron is the owner's lever; not read, not touched. **Log size:** this entry adds to the run log; `check-log-size`'s MEASURED line on the next run is the figure to quote. **Backlog:** 0 b added — the residual above is a note under this entry, per W-6.2 rule 2, not a numbered item.
