...
### 2026-09-06 (scheduled dev-agent, W-5.3 archiving pass — the seventh, and the run log was 7 commits from the budget at which no run can commit anything) — the pass was routine; what was not is that the record I wrote about it measured the region before inserting itself into it, which is the exact defect W-7.2 was written about, and I did it three times before deleting the number

**Where the pick came from, and the premise re-measured rather than taken on report.** The previous
entry queued the archiving pass as "the cheapest and unblocked", reporting the run log at **279,499 b**.
Re-measured at run start: **289,013 b** — the note was 9,514 b stale, in the direction that matters.
W-6.2 rule 1 does not bind: W-5.3 archiving is a standing rule with a measured trigger, which the clause
itself calls a legitimate whole run, and the previous pick was a filed note rather than a residual chain.
**Why it outranked learner-visible work, which is the only interesting part of the pick.** The run log's
FAIL budget is 350,000 b, growth is **+8,392 b/commit net**, and `(350,000 − 289,013) / 8,392` is
**7.3 commits** — at this cadence, under a day. **At that point `npm test` exits 1 and no run can commit
anything, including content fixes.** That is the argument; "the log is untidy" is not.

**Step 3.5 — the instrument was proven in both directions before it touched a real file.**
- **Plants, on a scratchpad copy:** 14 `MOVE` markers (one per 09-05 entry) and 16 `KEEP` (one per
  09-06). Pre-move: live 14/16, archive 0/0, and a **dead plant never inserted returns 0** — so a later
  "not found" is a real absence and not a broken pattern.
- **Post-move on that copy:** live MOVE **0**, archive MOVE **14**, live KEEP **16**, archive KEEP **0**,
  each of the 14 landing **exactly once**.
- **Both refusal guards fired for the right reason**, read off the message and not the exit code: the
  newest day → *"is not the OLDEST/tail block … would strand older days"*; an absent day → *"not
  present"*. Both left both files byte-identical. **And an acceptance control ran alongside them** — the
  valid day changed the files — so the refusals are real rather than blanket.

**Verification on the real files, sourced from the repo rather than my own buffer.**
`git show HEAD:AGENT_LOG.md` is the pre-cut copy, so a reviewer who was not present can reproduce all of
this from the repo alone. **Containment 14/14** byte-identical, present exactly once in the archive and
**zero times** in the live log; 16 retained. **Two plants proved the containment check can fail** — one
character changed inside a moved entry, and a whole 10,458 b entry deleted from the archive — both
**13/14, exit 1**, both written to scratchpad copies and never to the real archive, and each plant was
confirmed to have *landed* before its result was read. **Whole-corpus control:** the non-blank-line
multiset across BOTH files is identical before and after (md5 `6789576d26075f0828c86ad580741d52`), the
only exclusion being the intended new `## Archived 2026-09-05` heading; 4 blank lines normalized at entry
boundaries. **The never-archived floor is byte-identical** (md5 `2dccb4a5c19403fa694e1919cd6f97a8`), so
nothing archivable was taken out of a never-archived section.
**Ordering — the defect the sixth pass shipped and caught by hand.** The archive reads oldest-first,
reversing the live log's newest-first. My mover reverses in code, and I checked the result against an
**independent source rather than against my own reversal**: `git log` timestamps for 09-05 give oldest
`ba7fd0d` 00:15 and newest `a01246b` 22:19, and the section opens on the first and closes on the second
(item 142's rule — order by commit timestamp, not file position). 14 commits, 14 entries.
**No checker lost visibility**, re-read rather than inherited: `check-measurements.mjs:67` sets
`LOGS = [AGENT_LOG.md, AGENT_LOG.archive.md]` and reads both; `check-backlog.mjs:29` reads only
`AGENT_LOG.md`'s backlog, proven byte-identical; `check-claims.mjs` reads neither.
**Numbers:** run log **289,013 → 141,323 b** (115.6% → 56.5% of warn), headroom **7.3 runs from FAIL →
12.1 runs**. The archive's stale title range was corrected `2026-09-04 → 2026-09-05` (one line, diffed) —
it had rotted through two previous passes. `npm test` **0 failures, 4 warnings**; the fifth warning is
gone and removing it was the point. `npm run build` green in **1.07s**, bundle `index-B1mndoLB.js` at
**264,930 b — the same hash and size the live site was verified against**, which is the correct result
for a change touching 0 files under `src/`.

**W-7.2 rules 1-2 applied to W-5.3 while I was in it, and this is the substantive half of the run.**
The clause carried a **false sentence** — the 2026-09-03 note claiming the oldest-first convention was
*"now covered by an asserted order control"* — followed 50 lines later by a 13-line ⛔ block quoting it
in order to retract it. That is precisely the "ORIGINAL CLAUSE, kept because the retraction refers to it"
pattern W-7.2 rule 2 retires. **Fixed at source and the retraction deleted** (−574 b): the convention now
states, in place, that nothing enforces it but a reader, that `npm test` passes 0 failures on an inverted
section, and what to do about it. **The record of having been wrong is preserved** (rule 3) — the
sixth pass's full account is in the archived 2026-09-05 entry and in commit `679b701`, cited rather than
pasted. ⛔ **I reworded no clause of the RULE itself** — the trigger and action clauses stay the owner's,
per W-5.3.

**⭐ Step 5 — adversarial self-check, and it caught a real defect in my own writing rather than nothing.**
*Blindspot register:* `git diff --name-only -- src/ content/ public/ locales/ index.html` is **0 files** —
the diff is two Markdown files — so §10.1/§10.2/§10.3 and the §2.3 stale-data rule have a byte-identical
corpus, and the unchanged bundle hash is the evidence rather than a grep over untouched text;
`check-blindspot` passes with its own timing control firing. *DECISIONS.md conflict:* re-grepped with a
**two-sided control** (a term that IS in the file returns 12, a nonsense term returns 0) — `archiv`
appears **nowhere**, and every `AGENT_LOG` hit is a pointer to an item, so no closed decision is touched.
*Already-done item:* no — this is the recurring standing action, the seventh firing, and it is
deliberately **not** a backlog-compression pass, which is the only thing that can move the floor and is
item 115's half. *W-6.3:* `git diff --stat -- scripts/` is **empty** — zero lines of instrument code; the
mover lives in the scratchpad, uncommitted, and the case for promoting it is filed as a note under W-5.3
rather than acted on.
⛔ ***My own verification claim — where it broke, and it broke twice.***
**(a) An exit code read through a pipe.** My first DECISIONS.md check ran `grep … | head` and then
printed `$?`, which is `head`'s status, not `grep`'s — it reported "exit 0" under a label claiming that
meant no hits. Re-run direct, with the two-sided control above. This is a failure mode this repo has
already recorded, and I walked into it anyway.
**(b) The one worth reading, because it is W-7.2's own defect reproduced by the run applying W-7.2.**
The seventh-pass record I wrote states the floor "untouched at 453,677 b" — a figure I had measured
**before writing 2,075 b into the floor**. I corrected it; the correction changed the number again. I
corrected it a second time; it changed again. **Three iterations, each one stale on arrival**, which is
the same shape as W-7.2 rule 4's finding that the review measured the priority-block region before
inserting itself into it. **The fix was not a fourth figure — it was deleting the figure** and pointing
at `check-log-size.mjs`'s generated MEASURED line, which is the App summary's standing rule ("no count
that a script generates") applied to a region for the first time rather than to a lesson count.
**The transferable version: a self-referential measurement cannot be corrected, only delegated.**
...
1. **Promoting the archiving mover to `scripts/`** — full note under W-5.3, with the counter-argument.
   It is not due as a *check* (no learner-visible failure); it may be due as *automation of a standing
   action*, which is the `clean-tree`/`deploy` precedent. Next pass decides deliberately.
2. **This run's cost, under W-7.2 rule 5.** The backlog moved by this run's writing only; the archiving
   move took **0 b** out of it. **Do not read a total off this line** — `npm test`'s MEASURED line is
   the instrument, and the baseline to beat on 2026-09-13 is **425,473 b**.

**Top item for the next run:** ⛔ owner-only at the head, unchanged — **the Netlify token** (`npm run
deploy` refuses until it exists, so deploying is still a manual drag), then **O-2** (an analytics
account; the app is live and current and nothing can yet say whether one person has opened it).
**For a scheduled run: archiving is NOT due again — 12.1 runs of headroom — do not pick it, and do not
read this line as a handoff.** Go back to learner-visible work: the app is live *and* serving HEAD for
the first time, so a content or UX fix now actually reaches a reader.
⚠️ **W-7.3's clock:** `market.json` is `asOf 2026-09-04`, age 2 days, and Sectors goes to the
unavailable state on **2026-09-09** — owner's scheduled job, flagged not touched, and `npm test` says
so on every run.

**Owner tree:** the owner's untracked `UIUX/` and `course` were **untouched** — not read, not edited,
not staged. No file under `src/`, `content/`, `public/` or `locales/` was touched;
`public/data/market.json` untouched at `asOf=2026-09-04`. `HEAD` re-checked before writing and unmoved
at `b36ec80`.
