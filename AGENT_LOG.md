...
### 2026-08-25 (owner-directed) — the money supply (M0/M1/M2), which the app had been referring to for three weeks without defining

**Owner request: "add m money(M0,M1,M2) supplies for economic topics."** Item 113 was paused for it.

#### The gap this closes, and where it came from

`grep -rniE "money supply|monetary base|\bM[012]\b" src/content/` returned **three** hits before this
run and **no definition among them**: the `Credit` glossary entry, the economy-track credit lesson,
and that lesson's quiz explanation all say *"total credit outstanding is many times larger than the
base money supply"*. The phrase was doing teaching work that no screen had done.

**It got there as a fix, which is why it stayed unexamined.** The 2026-08-02 run replaced the dated
figures `~$50T total credit vs ~$3T actual money` with the figure-free "many times larger than the
base money supply" framing (see the "Stale/dated factual figures reworded" entry). That was correct
under §2.3 — and it left a term of art in the copy with nothing behind it.

#### What shipped

- **`src/content/glossary.js`** — three entries, **M0 / M1 / M2**, 5 languages each (35 → 38 terms).
  Keys are the numerals so they are searchable as typed; `s` carries the spelled-out name so
  searching "money supply" finds M1 and M2. **Verified live in the running app**: searching `M2`
  returns one row, `money supply` returns two.
- **`src/content/markets.js`** — `moneyAggregates` (3 tiers: what is inside each, and who creates
  it) and `moneySupplyCaption`, 5 languages.
- **`src/screens/reference/MarketSignals.jsx`** — renders the tiers as an **`<ol role="list">`**
  placed directly after QE/QT. An ordered list, not the `<ul>` the principles below use: these three
  are defined in terms of each other and reading M2 before M1 does not work. `role="list"` for
  check-data.mjs §20's reason (WebKit drops list semantics under `list-style: none`).
- **`LAUNCH_PLAN.md`** — the §1 asset sentence's glossary count, regenerated by `npm run readiness
  -- --write` rather than hand-edited. A guard caught the stale `35` before I did.

#### Accuracy notes, since this is teaching copy

- **Savings deposits are in M1, not M2**, under current US definitions — a lot of older material has
  this the other way round, and the Fed's own composition changed. The copy says "checking and —
  under current US definitions — savings accounts", and every entry carries the caveat that **each
  central bank sets its own composition and has redefined it over time**, so long historical
  comparisons need care. That is also what keeps the entries from going stale.
- **The nesting asymmetry is stated, not implied by the layout**: M1 sits inside M2, but the base
  sits inside neither — they share physical cash, while reserves count only in the base. This is the
  part readers get wrong, and it is the answer to the question a reader has just after watching the
  balance sheet grow: *is there more money now?* Not necessarily.
- **§2.3 respected by construction**: no level, no growth rate, no date, no currency figure. The
  aggregates are taught as *composition*, which is the part that does not go stale — deliberately
  not reintroducing the figures the 2026-08-02 run removed.
- **§10.1**: every line describes a mechanism. Nothing says what a reader should do about it.

#### Verification

`npm run build` clean. `npm test` **0 failures, 2 warnings** (the documented translation baseline).
The stale-count failure above was a real guard firing on a real staleness and is fixed, not muted.

**Live, against the rebuilt `dist/`** (bundle read back in the browser as `index-DZQnuaq3.js`): the
Market Dashboard renders the section between "Yield Curve Shapes" and "Key Principles", the three
tiers and the caption all read correctly, and the `ja` copy renders in full.

**a11y matrix, the instrument built for exactly this** — `MarketSignals.jsx` is the
`reference-markets` state. `A11ySweep.selftest()` **PASS 9/9**, `A11yStates.selftest()` **PASS**,
then **all 5 languages × 13 states: every state reached (`missed: 0`), 0 findings.**
`reference-markets` now reads heading sequence **`12222`** (was `1222` — one new `<h2>`, which is
the change), and its arrival assertion still holds in every language.

#### Step 5 — adversarial self-check

- **Blindspot register** — no regression. Across 124 added `src/` lines: advice-adjacent phrasing
  **0**, Dalio/`principles of` **0**, `Month YYYY` **0**, currency figures or percentages **0**.
  **Control**: `M0|M1|M2|reserve` returns **29**, so the grep reaches them. One bare `2026` — a code
  comment in `markets.js`, which *is* a bundled module, so it was checked against the artifact
  rather than assumed stripped: `grep -rl "2026-08-25" dist/assets/` → **0**, with **controls**
  `Monetary base` → **1 file** and `マネタリーベース` → **1 file**, proving the grep reaches bundle
  content in both scripts.
- **`DECISIONS.md` conflict** — none. Content stays in `.js` modules, 5-language parity enforced by
  §4; no dependency, config, routing or storage change.
- **Already-done backlog item** — no. `git log --all -S'moneyAggregates'` → **0**. **Control**:
  `-S'balanceSheetHistory'` → **2**, so the pickaxe reaches this shape.
- **A link I did NOT add, and why.** The obvious finish is a `lessonTerms.js` chip pointing the
  credit lesson's "base money supply" at the new M0 entry. §17 forbids it correctly: it requires the
  section's English text to mention the key or its `en.s`, and "base money supply" is neither "M0"
  nor "Monetary Base (M0)". A chip the prose does not name is a chip the reader cannot connect.
  Making it legitimate means rewording that lesson body in five languages, which is a content change
  the owner did not ask for. **Filed as item 114.**

#### Next

- **Item 113** (paused, not dropped): the `check-data.mjs` guard for hardcoded English in
  `a11y-states.js` recipes. `check-data.mjs` is free again as of `b6c9bc9`.
- **Item 114** (filed this run): reword the credit lesson's "base money supply" to "monetary base"
  in 5 languages so the §3.0.3 glossary chip becomes legitimate — small, and it closes the loop this
  run opened.
- **Unchanged and still the entire critical path, both owner-blocked: O-1** (a deployed URL) and
