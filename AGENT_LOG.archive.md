...
**Archiving rule, for future weekly reviews:** at each Sunday review, move run-log entries older
than the *previous* review boundary into this file, appending them in date order under a new
`## Archived <date range>` heading. Keep one full review period live in `AGENT_LOG.md`.
