...
each one reads that file to orient. Nothing here is deleted or edited; these are the original
entries, moved verbatim.

**This file is history. You do not need to read it to pick up work** — the live
`AGENT_LOG.md` carries the App summary, the prioritized backlog, the Environment note, and every
run-log entry from 2026-08-09 (the last weekly-review boundary) onward. Come here when you need
the detail behind a completed item, or to check whether something was already tried.

**Archiving rule, for future weekly reviews:** at each Sunday review, move every run-log entry
dated **before the most recent review boundary** into this file, appending it in date order under a
new `## Archived <date range>` heading. What stays live in `AGENT_LOG.md` is the current review
period — the entries written since the last review — plus the App summary, the backlog and the
Environment note, which are never archived.

*(Reworded 2026-08-17, on the rule's first reuse. It previously said "older than the *previous*
review boundary", which read as one period further back than intended and contradicted its own
"keep one full review period live" clause. Concretely: at the 2026-08-16 review the boundaries were
08-09 and 08-16, and the entries to move were 08-09→08-15 — everything before the newest boundary,
not before the one before it. Nothing about the intent changed; the wording just could not be
followed literally.)*

Covers **the first 21 dev-agent runs plus three weekly reviews**: the initial scaffold, the
blindspot-register fixes (§10.1/§10.2/§10.3), the Markets stale-date fix, the reproducible build
environment, the four-step JSX monolith split, the data-shape check harness, the first-session
flow (6a–6e), the accessibility pass, the 2026-08-04 rebuild onto `src/`, the market-data
pipeline and Sector-performance screen, the launch-readiness scorecard, the automated blindspot
checks, and lessons 18–37.

## Archived 2026-08-01 → 2026-08-08
...
## Archived 2026-08-09 → 2026-08-15

Appended 2026-08-17 (owner-requested), the second application of the rule above. `AGENT_LOG.md` had
grown back to 821 KB / 9,007 lines — it had been cut to 516 KB the previous day, so this is 305 KB
of run log written in about twenty-four hours. That rate is the argument for the rule, not against
it: the file is read by every run, and a week of entries at this pace costs more to carry than the
history is worth having inline.
